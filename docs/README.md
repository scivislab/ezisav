# EZ-ISAV: Developing and Evaluating In Situ Visualization Algorithms using Containers
...
We present EZ-ISAV, a collection of layered Docker containers encapsulating several pipelines for multiple in situ frameworks in a ready to use package, which are thus quickly operational.
...
We present EZ-ISAV, a set of workflows which aims to alleviate this problem. 
...
Using the EZ-ISAV examples, the time spent on setting up lengthy workflows and integrating in situ visualization into simulations can instead be used for additional validation and evaluation. 
